...
public class Main {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for (int i = 0; i < array4.length; i++) {
            int binaryResult = binarySearch(array3, array4[i], 0, array3.length - 1);
        ...
        System.out.println("Found " + (binaryCount) + " / " + 500 + " entries.");

        long quickResults = quickEnd - quickStart;
        long binaryResults = binaryEnd - binaryStart;
        long quickAndBinary = quickResults + binaryResults;

        long minutesQuick = TimeUnit.MILLISECONDS.toMinutes(quickResults);
        long secondsQuick = TimeUnit.MILLISECONDS.toSeconds(quickResults) - (TimeUnit.MILLISECONDS.toMinutes(quickResults) * 60);
        long millisecondsQuick = quickResults - (TimeUnit.MILLISECONDS.toSeconds(quickResults) * 1000);

        long minutesBinary = TimeUnit.MILLISECONDS.toMinutes(binaryResults);
        long secondsBinary = TimeUnit.MILLISECONDS.toSeconds(binaryResults) - (TimeUnit.MILLISECONDS.toMinutes(binaryResults) * 60);
        long millisecondsBinary = binaryResults - (TimeUnit.MILLISECONDS.toSeconds(binaryResults) * 1000);

        long minutesQuickAndBinary = TimeUnit.MILLISECONDS.toMinutes(quickAndBinary);
        long secondsQuickAndBinary = TimeUnit.MILLISECONDS.toSeconds(quickAndBinary) - (TimeUnit.MILLISECONDS.toMinutes(quickAndBinary) * 60);
        long millisecondsQuickAndBinary = quickAndBinary - (TimeUnit.MILLISECONDS.toSeconds(quickAndBinary) * 1000);


        System.out.println("Time taken: " + minutesQuickAndBinary + " min." + " " + secondsQuickAndBinary + " sec." + " " + millisecondsQuickAndBinary + " ms.");
        System.out.println("Sorting time: " + minutesQuick + " min." + " " + secondsQuick + " sec." + " " + millisecondsQuick + " ms.");
        System.out.println("Searching time: " + minutesBinary + " min." + " " + secondsBinary + " sec." + " " + millisecondsBinary + " ms.");
...
//    public static void timeTaken(long milliseconds) {
//
//        long minutesLinear = TimeUnit.MILLISECONDS.toMinutes(milliseconds);
//        long secondsLinear = TimeUnit.MILLISECONDS.toSeconds(milliseconds) - (TimeUnit.MILLISECONDS.toMinutes(milliseconds) * 60);
//        long millisecondsLinear = milliseconds - (TimeUnit.MILLISECONDS.toSeconds(milliseconds) * 1000);
//
//
//    }
